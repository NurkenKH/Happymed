...
from models import Appointment, Doctor, MedHistory, Specialization, db, Patient, User, Administrator
...
def make_appointment(data):
    try:
        ...
        date = datetime.strptime(time_slot['date'], '%Y-%m-%d').date()  # Updated date format
...
def get_appointments_for_doctor(email):
    try:
        ...
        appointments_list = [{
            ...
            'appointment_type': appointment.Appointment.appointmentType,
...
def get_appointments_for_patient_with_pid(user_id):
    try:
        ...
        # Fetch appointments and join with Doctor table
        appointments = db.session.query(Appointment, Doctor).join(Doctor, Appointment.doctor_id == Doctor.doctor_id).filter(Appointment.patient_id == patient_id).all()
...
def complete_appointment(data):
    try:
        ...
        # Add appointment details to MedHistory
        medhistory_entry = MedHistory(
            appointment_id=appointment.appointment_id,
            diagnosis=diagnosis,
            feedback=feedback,
        ...
        db.session.add(medhistory_entry)
        db.session.commit()
...
def get_medhistory(email):
    try:
        ...
        for appointment in appointments:
            ...
            doctor = appointment.doctor if appointment.doctor else None
            doctor_name = (
            ...
            medhistory_entry = MedHistory.query.filter_by(appointment_id=appointment.appointment_id).first()
            diagnosis = medhistory_entry.diagnosis if medhistory_entry else None
            feedback = medhistory_entry.feedback if medhistory_entry else None
...
def get_feedback(appointment_id):
    try:
        # Fetch the MedHistory entry
        medhistory_entry = MedHistory.query.filter_by(appointment_id=appointment_id).first()
        if not medhistory_entry:
            return {'error': 'Medical history entry not found'}, 404

        return {
            'diagnosis': medhistory_entry.diagnosis,
            'feedback': medhistory_entry.feedback
        }, 200
    except Exception as e:
        return {'error': str(e)}, 400

def edit_feedback(appointment_id, data):
    try:
        new_feedback = data.get('feedback')
        new_diagnosis = data.get('diagnosis')
        
        if not new_feedback and not new_diagnosis:
            return {'error': 'Feedback or diagnosis content is required'}, 400
        ...
        # Update feedback and diagnosis if provided
        if new_feedback:
            medhistory_entry.feedback = new_feedback
        if new_diagnosis:
            medhistory_entry.diagnosis = new_diagnosis

        db.session.commit()

        return {'message': 'Feedback and diagnosis updated successfully'}, 200
    except Exception as e:
        db.session.rollback()
        return {'error': str(e)}, 400
def cancel_appointment_by_doctor(data):
    try:
        appointment_id = data['appointment_id']
        ...
        # Update the status to "canceled by doctor" and set the cause
        appointment.status = 'canceled by doctor'
        appointment.additional_text = cause
        db.session.commit()

        return {'message': 'Appointment canceled by doctor successfully'}, 200
    except Exception as e:
        db.session.rollback()
        return {'error': str(e)}, 400
def get_cause(appointment_id):
    try:
        # Fetch the appointment
        appointment = Appointment.query.filter_by(appointment_id=appointment_id).first()
        if not appointment:
        ...
        # Fetch the additional_text (cause)
        cause = appointment.additional_text
        return {'cause': cause}, 200
    except Exception as e:
        return {'error': str(e)}, 400