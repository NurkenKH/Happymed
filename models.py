...
class MedHistory(db.Model):
    ...
    appointment = db.relationship('Appointment', back_populates='medhistory')
...
class Patient(db.Model):
    ...
    phone = db.Column(db.String(20), nullable=True)
    gender = db.Column(db.Enum('Male', 'Female'), nullable=True)
    medical_history = db.Column(MutableList.as_mutable(JSON), default=[])
...
class Appointment(db.Model):
    ...
    status = db.Column(db.Enum('completed', 'booked', 'canceled', 'canceledP', 'canceled by doctor'), default='booked', nullable=False)
    appointmentType = db.Column(db.Text, nullable=True)
    additional_text = db.Column(db.String(255), nullable=True)

    doctor = db.relationship('Doctor', backref=db.backref('appointments', lazy=True))
    patient = db.relationship('Patient', backref=db.backref('appointments', lazy=True))
    medhistory = db.relationship('MedHistory', back_populates='appointment', uselist=False)  # Add this line